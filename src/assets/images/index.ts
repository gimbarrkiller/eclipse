export * from './icons';
export * from './banners';

export { default as avatarImage } from './avatar.png';
export { default as logoImage } from './logo.svg';
export { default as mainLaptopImage } from './main_laptop.png';
export { default as mainLaptopImageTablet } from './main_laptop2.png';
export { default as autosImage } from './autos.png';
export { default as dashboardImage } from './dashboard.png';
export { default as dashboardMobileImage } from './dashboard-mobile.png';
export { default as emailSuccessImage } from './email_success.png';
export { default as newsBannerImage } from './newsBanner.png';

export { default as disconnectImage } from './404.png';

export { default as illustrationVideoImage } from './illustrationVideo.png';
export { default as illustrationChatImage } from './illustrationChat.png';
export { default as illustrationNoteImage } from './illustrationNote.png';
export { default as illustrationPuzzleImage } from './illustrationPuzzle.png';
export { default as illustrationTgImage } from './illustrationTg.png';
export { default as illustrationTgLightImage } from './ittustrationTgLight.png';
export { default as illustrationNoRef } from './illustration_no_ref.png';
export { default as illustrationNoTransaction } from './illustrationNoTransaction.png';
export { default as illustrationNoTickets } from './illustrationNoTickets.png';

export { default as aboutUsImage } from './aboutUs.webp';
export { default as marketingPlanImage } from './marketingPlan.webp';
export { default as rewardsImage } from './rewards.webp';
